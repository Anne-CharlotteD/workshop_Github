...
plot_calibration_exclusions(final_results %>% filter(MAC_mix == 41))

plot_calibration_exclusions(final_results %>% filter(MAC_mix == 42 | MAC_mix == 36))
...
write.csv(cal_curve_table, paste("output/Apolar_validation_fake/cal_curve_table_fake_", validation_plate, ".csv"), row.names = FALSE)
